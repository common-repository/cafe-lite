...
abstract class CleverWidgetBase extends Widget_Base
{
    ...
    protected function getViewTemplate($tpl_slug, $tpl_name, $settings = [])
    {
        ...
        if ($located) {
            ...
            /* translators: %s slug template and template name */
            printf(esc_html__('Failed to load template with slug "%1$s" and name "%2$s".', 'cafe-lite'), esc_html($tpl_slug), esc_html($tpl_name));
        }
    }
    ...
    protected function getSavedElementorTemplates()
    {
        ...
        if (!empty($posts)) {
            foreach ($posts as $post) {
                $tpls[$post->ID] = sanitize_text_field($post->post_title);
            }
    ...
    {
        if ($url != '') {
            $zoo_product_video_url = wp_parse_url($url);
