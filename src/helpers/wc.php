...
if (!function_exists('cafe_ajax_product_filter')) {
    function cafe_ajax_product_filter()
    {
        ...
        if (isset($_POST['product_attribute']) && isset($_POST['attribute_value'])) {
            if (is_array($_POST['product_attribute'])) {
                foreach ($_POST['product_attribute'] as $key => $value) {
                    $tax_query[] = array(
                        'taxonomy' => sanitize_text_field($value),
        ...
        if (isset($_POST['price_filter']) && $_POST['price_filter'] > 0) {
            $min = (intval(sanitize_text_field($_POST['price_filter'])) - 1) * intval(sanitize_text_field($_POST['price_filter_range']));
            $max = intval(sanitize_text_field($_POST['price_filter'])) * intval(sanitize_text_field($_POST['price_filter_range']));
            $meta_query[] = array(
                'key' => '_price',
                'value' => array($min, $max),
                'compare' => 'BETWEEN',
        ...
        if (isset($_POST['s']) && $_POST['s'] != '') {
            $wc_attr['s'] = sanitize_text_field($_POST['s']);
        }
