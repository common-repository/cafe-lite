...
final class Plugin
{
    ...
    private function preActivate()
    {
        if (version_compare(PHP_VERSION, '7.4', '<')) {
            throw new Exception('This plugin requires PHP version 7.4 at least!');
        }

        if (version_compare($GLOBALS['wp_version'], '6.4.1', '<')) {
            throw new Exception('This plugin requires WordPress version 6.4.1 at least!');
        }

        if (!class_exists('Elementor\Plugin')) {
            throw new Exception('This plugin requires Elementor Page Builder version 3.17.3 at least. Please install and activate the latest version of Elementor Page Builder!');
        }
    }
}
